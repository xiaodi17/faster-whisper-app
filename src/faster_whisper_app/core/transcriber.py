...
class FasterWhisperTranscriber:
    ...
    def transcribe_file(self, audio_path: str, beam_size: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Transcription completed: '{full_text[:100]}{'...' if len(full_text) > 100 else ''}'"
            )
            return result
    ...
    def transcribe_audio_data(
        self, audio_data: bytes, sample_rate: int = 16000, beam_size: int = 1
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🎯 Starting transcription of {len(audio_data)} bytes of audio data"
            )
            ...
            logger.info(
                f"📝 Result: '{full_text[:100]}{'...' if len(full_text) > 100 else ''}'"
            )
            return result
