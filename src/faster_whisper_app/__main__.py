...
from .core.exceptions import AudioRecordingError, ModelLoadError, TranscriptionError
...
class SpeechToTextApp:
    ...
    def initialize_components(self) -> bool:
        ...
        try:
            ...
            self.recorder = AudioRecorder(
                sample_rate=self.config.sample_rate, channels=self.config.channels
            )
                ...
                if self.config.audio_device_index is not None:
                    ...
                    if configured_device:
                        ...
                        self.terminal.show_status(
                            f"🎤 Configured to use device {self.config.audio_device_index}: {configured_device['name']}"
                        )
                    else:
                        self.terminal.show_error(
                            f"Configured device {self.config.audio_device_index} not found!"
                        )
            ...
            self.hotkey_handler = HotkeyHandler(
                callback=self.toggle_recording, hotkey=self.config.hotkey
            )

            self.terminal.show_status(
                "✅ All components initialized successfully!", "green"
            )
            return True
    ...
    def stop_recording(self) -> None:
        ...
        try:
            ...
            if audio_data:
                ...
                threading.Thread(
                    target=self._type_to_active_app_async, args=(result,), daemon=True
                ).start()

                # Log overall timing
                total_processing_time = time.time() - processing_start
                logger.info(
                    f"🏁 Total processing pipeline: {total_processing_time * 1000:.1f}ms"
                )
                logger.info(
                    f"📊 Breakdown - Audio: {audio_time * 1000:.1f}ms, Transcription: {transcription_time * 1000:.1f}ms, Display: {display_time * 1000:.1f}ms"
                )
                ...
                if "timing" in result:
                    ...
                    logger.info(
                        f"🔍 Transcription breakdown - WAV prep: {timing['wav_prep_time'] * 1000:.1f}ms, Model: {timing['model_time'] * 1000:.1f}ms, Segments: {timing['segment_time'] * 1000:.1f}ms"
                    )
    ...
    def _type_to_active_app_async(self, result: dict):
        ...
        try:
            ...
            logger.info(
                f"⌨️  Starting text output: '{text[:50]}{'...' if len(text) > 50 else ''}'"
            )
            ...
            logger.info(
                f"⚡ Text output timing - Prep: {prep_time * 1000:.1f}ms, Script: {script_time * 1000:.1f}ms, Total: {total_typing_time * 1000:.1f}ms"
            )
            ...
            logger.warning(
                f"⚠️  AppleScript execution timed out after {total_time * 1000:.1f}ms"
            )
        except Exception as e:
            total_time = time.time() - typing_start
